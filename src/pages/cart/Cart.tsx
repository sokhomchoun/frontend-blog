                    ...
                    <div className="grid lg:grid-cols-2 gap-12">
                                            ...
                                            <div className="flex gap-3">
                                                <div className="checkbox flex items-center">
                                                    <input 
                                                        ...
                                                        type="checkbox"
                                                        checked={isCheckedProduct.includes(element.product_id)}
                                                        onChange={() => handleCheckboxChange(element.product_id)}
                                                    />
                                                </div>

                                                <div className="flex items-center gap-4">
                                                    <div className="w-24 h-24 shrink-0 bg-white p-2 rounded-md relative">
                                                        <p className="absolute bg-red-500 text-white text-[10px] p-[2px] top-1">-{element.discount} %</p>
                                                        <img src={`${img_url}${element.product_img}`} alt="" className="w-full h-full object-contain" />
                                                    </div>

                                                    <div className="w-full">
                                                        <h3 className="text-base font-semibold text-gray-800">{element.product_name}</h3>
                                                        <div className="flex gap-2">
                                                            <h6 className="text-sm text-red-500 font-bold cursor-pointer mt-0.5">
                                                                $ {
                                                                    ((parseFloat(element.price) - ((parseFloat(element.price) * parseInt(element.discount) / 100))) * parseInt(String(element.quantity))).toFixed(2)
                                                                }
                                                            </h6>
                                                            <h6 className="text-sm text-black font-bold cursor-pointer mt-0.5 line-through">
                                                                $ {
                                                                    ((parseFloat(element.price)) * parseInt(String(element.quantity))).toFixed(2)
                                                                }
                                                            </h6>
                                                        </div>
                                                        <div className="flex gap-4 mt-4">
                                                            <div className="flex gap-2 items-center">
                                                                <button type="button"
                                                                    ...
                                                                    <span className="mx-2.5">{element.quantity}</span>
                                                                ...
                                                                <div className="flex gap-2">
                                                                    { element.size && (
                                                                        <h6 className="font-semibold text-[12px]">Size: <span className="font-bold">{element.size}</span></h6>
                                                                    )}
                                                                    { element.size && (
                                                                        <h6 className="font-semibold text-[12px]">Color: <span className="font-bold">{element.color}</span></h6>
                                                                    )}
                                                                </div>
                                                            </div>
                        ...
                        <form>
                            ...
                            <ul className="text-gray-800 mt-8 space-y-4">
                                <li className="flex flex-wrap gap-4 text-sm">Subtotal <span className="ml-auto font-bold">$ {(AfterPrice).toFixed(2)}</span></li>
                                ...
                                <hr className="border-gray-300" />
                                <li className="flex flex-wrap gap-4 text-sm font-bold">Total <span className="ml-auto">$ {(AfterPrice).toFixed(2)}</span></li>
                            </ul>

                            <button type="button" className="mt-8 text-sm px-4 py-3 w-full font-semibold tracking-wide bg-[#007bff] hover:bg-blue-600 text-white rounded-md">Make Payment  </button>
                        </form>
                    </div>
                </div>
            <Footer />
